...
use crate::{
    Data, DataRef, HeaderRow, Metadata, Ods, Range, Reader, ReaderRef, Xls, Xlsb, Xlsx,
    open_workbook, open_workbook_from_rs,
};
use std::borrow::Cow;
use std::fs::File;
    ...
    RS: std::io::Read + std::io::Seek,
{
    ...
    fn vba_project(&mut self) -> Option<Result<Cow<'_, VbaProject>, Self::Error>> {
        match self {
            Sheets::Xls(e) => e.vba_project().map(|vba| vba.map_err(Error::Xls)),
            Sheets::Xlsx(e) => e.vba_project().map(|vba| vba.map_err(Error::Xlsx)),
            Sheets::Xlsb(e) => e.vba_project().map(|vba| vba.map_err(Error::Xlsb)),
            Sheets::Ods(e) => e.vba_project().map(|vba| vba.map_err(Error::Ods)),
        }
    }
